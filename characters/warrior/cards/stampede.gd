...
func get_dynamic_description(modifiers: ModifierHandler, target: Node = null) -> String:
    ...
    if is_inked():
        return "Deal ? damage. If you rolled at least 5 Dice this turn, deal it twice" + suffix
    if not has_active_roll():
        return "Deal X damage. If you rolled at least 5 Dice this turn, deal it twice" + suffix
    var total := apply_target_modifier(modifiers.get_modified_value(Global.roll_value, Modifier.Type.DMG_DEALT), target)
    return ("Deal %d damage. If you rolled at least 5 Dice this turn, deal it twice" % total) + suffix
