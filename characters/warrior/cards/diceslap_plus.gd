...
func apply_effects(targets: Array[Node], modifiers: ModifierHandler) -> void:
    ...
    var base_damage = Global.roll_value + Global.roll_history.size() * 4
...
func get_dynamic_description(modifiers: ModifierHandler, target: Node = null) -> String:
    if is_inked():
        return "Deal ? damage, plus 4 for each consecutive Dice roll"
    if not has_active_roll():
        return "Deal X damage, plus 4 for each consecutive Dice roll"
