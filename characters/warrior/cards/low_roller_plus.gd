...
# Low Roller+ : 20 - X instead of 15 - X (base low_roller.gd). Inverted scaling, bigger ceiling.
...
func apply_effects(targets: Array[Node], modifiers: ModifierHandler) -> void:
    if targets.is_empty() or not has_active_roll():
        ...
        return
    var base := maxi(0, 20 - int(Global.roll_value))
