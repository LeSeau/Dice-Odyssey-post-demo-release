...
# The coin decides ON REVEAL: heads = X2 damage lands and your Power resets (emitted
# manually below - the SUPPORT rarity flag stops hand.gd's automatic reset, which would
# fire instantly at play and spoil the flip); tails = nothing happens and your Power
# stays banked for the next card. Damage is locked in at play time (modifiers included).
# Known edge: rolling again during the ~0.6s flip means a heads reveal resets the fresh
# roll too - short window, accepted for v1.
...
func apply_effects(targets: Array[Node], modifiers: ModifierHandler) -> void:
    if targets.is_empty():
        ...
        return
    var heads := randi() % 2 == 0
    var damage := modifiers.get_modified_value(Global.roll_value * 2, Modifier.Type.DMG_DEALT)
    var target: Node = targets[0]
    var tree := target.get_tree()
    Events.coin_flip.emit(heads, Global.last_played_card_position, target)
    note_delayed_hit(Global.COIN_FLIP_TIME)
