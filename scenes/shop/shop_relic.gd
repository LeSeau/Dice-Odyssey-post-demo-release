...
@onready var gold_cost := randi_range(75,150)
...
func update(run_stats: RunStats) -> void:
    ...
    if Global.gold >= gold_cost:
        price_label.text = "[center][color=#FFD700]" + str(gold_cost) + "G[/color][/center]"
        buy_button.disabled = false
    else:
        price_label.text = "[center][color=#FF4444]" + str(gold_cost) + "G[/color][/center]"
