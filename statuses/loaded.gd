class_name LoadedStatus
extends Status

# LOADED N: every Dice roll gains +N Power. The game's second scaling axis alongside Strength
# (Julien, 2026-08-16) - Strength scales per HIT, Loaded scales per ROLL, so they reward
...
#
# This resource is the BADGE ONLY - the effect lives in Global.loaded_amount because dice.gd
# reads it inside _apply_roll_result on every roll. Same display-status / global-effect split
# Emanation already uses.
#
# START_OF_TURN so the turn-scoped slice ("Gain Loaded 2 this turn") can expire and the badge
...
func apply_status(_target: Node) -> void:
    if Global.loaded_expiring > 0:
        Global.loaded_amount = max(0, Global.loaded_amount - Global.loaded_expiring)
        Global.loaded_expiring = 0
    # Re-sync rather than trusting the accumulated badge: StatusHandler.add_status() stacks the
    # delta when a card grants Loaded, but only this line can take the count back DOWN when a
    # turn-scoped slice ends.
    stacks = Global.loaded_amount
