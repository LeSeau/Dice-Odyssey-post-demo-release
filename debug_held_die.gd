...
func _run_render() -> void:
	...
	# HELD_DIE_TYPES cycles the hero through each dice type, holding long enough for the
	# retune flash and the punch to settle, and prints the frame each one lands on so the
	# contact sheet can pull a clean still per colour.
	var types := OS.get_environment("HELD_DIE_TYPES")
	if types == "":
		types = "blue,magma"
	var hold := 30
	if OS.get_environment("HELD_DIE_HOLD") != "":
		hold = int(OS.get_environment("HELD_DIE_HOLD"))
	...
	for part in types.split(","):
		...
		_set_type(player, t)
		await _frames(hold)
		print("[held_die] SETTLED %s frame %d" % [t, Engine.get_process_frames()])
