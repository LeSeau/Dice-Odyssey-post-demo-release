...
## TL;DR — où on en est (fin de session du 2026-07-03)

Lire ceci avant de re-creuser quoi que ce soit — ça évite de redécouvrir des choses déjà établies.

- **Cette session a couvert** : (1) **système de particules "power orbs"** (`dice.gd::_spawn_power_orbs()`) : quelques orbes volent du dé vers le nombre de Power à chaque roll, en suivant une courbe de Bézier bombée vers le haut ("rainbow" grossier, contrôle randomisé) plutôt qu'une ligne droite, colorés par type de dé, avec un compte/taille/luminosité qui montent avec le roll (bonus supplémentaire sur max roll) ; le nombre de Power reste mis à jour **immédiatement** au roll (`_apply_roll_result`) — une tentative de retarder l'affichage jusqu'à l'arrivée du premier orbe a été essayée puis rejetée par Julien — mais une réaction visuelle secondaire plus petite (`_play_power_orb_arrival_reaction`) se déclenche en plus, synchronisée sur l'arrivée du premier orbe (flash coloré + petit pop, distinct du punch/flash blanc du roll lui-même) ; (2) **highlight du bouton End Turn** (`battle_ui.gd::_update_end_turn_highlight`, pulse or lent) quand il ne reste plus aucune action possible ce tour : 0 dé restant (tous types), 0 power banké, ET aucune carte Celestial (`can_play_without_dice`) en main — n'importe laquelle des trois suffit à désactiver le highlight ; (3) **nouveau système de coloration des mots-clés** dans les descriptions de cartes ET de reliques — voir section dédiée "Descriptions colorées (mots-clés)" sous "Cartes" avant de toucher à `KeywordColorizer`, `tags`, ou aux Labels/RichTextLabel de description ; (4) conversion des Labels de description de carte (`card_ui.tscn`/`card_menu_ui.tscn`, y compris `BonusEffectLabel`) en `RichTextLabel` pour supporter le BBCode nécessaire à la coloration — a cassé pas mal de choses au passage (alignement, police/contour/ombre, un bug de survol "saccadé"), tout corrigé, voir section dédiée ; (5) unification des tooltips d'explication de dé (`tooltip.gd` du hover carte/relique + `dice_tooltip.gd` du shop) sur un seul texte partagé (`KeywordColorizer.DICE_TOOLTIP_TEXT`, format "Faces: X, Y, Z"), et ajout des 5 types de dés qui n'avaient encore jamais eu de tooltip nulle part (Blue/Red/Odd/Even/Mech — seuls Green/Magma/Giant/Evil en avaient un avant) ; (6) tentative de restylage visuel du `dice_shop.tscn`, **en grande partie annulée** à la demande de Julien (préfère l'ancien look pour le panel/les headers/la police) — seuls les styles normal/hover/pressed des boutons Buy/Reroll/Exit (qui n'avaient auparavant AUCUN style custom, juste le thème Godot par défaut) ont été gardés ; (7) petite amélioration de l'animation d'ouverture du trésor (`treasure.tscn`, squash d'anticipation + pop d'échelle ajoutés à l'animation "open" existante, aucun nouveau nœud/script).
- **Le plus gros morceau de cette session** : la coloration des mots-clés — voir section dédiée sous "Cartes". `custom_resources/keyword_colorizer.gd` (classe `KeywordColorizer`) est maintenant la source unique partagée entre `Card.get_colorized_description()` et `Relic.get_colorized_description()`, extraite dès qu'un 2e vrai consommateur (les reliques) a eu besoin exactement de la même logique — à réutiliser si un 3e type de description apparaît un jour plutôt que de dupliquer encore.
- **Piège corrigé cette session, à surveiller si un futur Label de carte/UI est converti en RichTextLabel** : `RichTextLabel` a `mouse_filter` par défaut à **STOP** (contrairement à `Label` qui est **IGNORE**) — si ce n'est pas explicitement remis à `mouse_filter = 2` (+ `scroll_active = false`), le nœud avale les événements souris et le survol/drag de tout ce qui est dessous devient saccadé. Bug vécu et corrigé sur `Description` et `BonusEffectLabel` dans `card_ui.tscn`/`card_menu_ui.tscn`.
...
- **Découverte importante (session précédente, toujours vraie)** : `card.requirement` (l'enum MIN/MAX/EXACT/etc.) était **purement cosmétique** avant la session du 2026-07-02 — uniquement utilisé pour le badge visuel et le tooltip, jamais vérifié pour bloquer le jeu d'une carte. `CharacterStats.can_play_card()` est un stub qui renvoie toujours `true`. Chaque carte gère elle-même sa condition via un `if` dans `apply_effects()` ; si la condition n'est pas remplie, la carte se joue quand même mais son effet ne fait rien (dé "gaspillé"). `Card.meets_requirement()` est la première utilisation réelle de cet enum pour de la logique (uniquement pour savoir si la description doit se résoudre, pas pour bloquer le jeu de la carte).
- **Décisions encore ouvertes** (héritées de sessions précédentes, voir section dédiée plus bas) : nom final de RITE (pas "Rite", trop religieux — "Glyph" a émergé comme piste favorite en fin de session mais **rien n'est encore appliqué au code**, l'enum est toujours `RITE` dans `card.gd`), icône visuelle RITE (en attente d'asset de Julien), 1-2 cartes "Keep" à couper jamais précisées, texture des 8 dés non-Blue.
