...
## TL;DR — où on en est (fin de session du 2026-07-03, session longue en plusieurs blocs)
...
- **⚠️ PROCHAINE SESSION = ÉQUILIBRAGE DES COMBATS → lire [balance_analysis_2026-07.md](balance_analysis_2026-07.md) EN PREMIER.** Analyse complète faite le 2026-07-04 (toutes les stats ennemies vérifiées dans le code, modèle mathématique D/P validé par Julien, plan de retune approuvé, ordre d'implémentation en §8). RIEN n'est encore implémenté. Points critiques à savoir avant de toucher quoi que ce soit : (1) `global.gd: gold = 7575` est un leftover de testing (vrai départ = 75) ; (2) `warrior_starting_deck.tres` contient 2 cartes de TEST (Dice Slap, Calculations) à remplacer par 2 Strikes — le vrai deck de départ est 4 Strike / 4 Block / Low Blow / Reinforce / Recombobulate ; (3) le kit du Goblin est mort (l'action toujours-vraie est le 1er enfant CONDITIONAL → il spamme 7 dmg à vie) ; (4) le tier 2 n'existe pas au runtime (mislabels + `run.gd::_get_tier_for_room` mappe tout sur tier 1) — la restructure en 3 tiers (coupure au trésor de l'étage 8) est APPROUVÉE par Julien ; (5) HP joueur réel = 66 (pas 70 comme d'anciennes versions de ce fichier le disaient).
...
- **Décisions encore ouvertes** (héritées de sessions précédentes, voir section dédiée plus bas) : nom final de RITE (pas "Rite", trop religieux — "Glyph" a émergé comme piste favorite mais **rien n'est encore appliqué au code**, l'enum est toujours `RITE` dans `card.gd`), icône visuelle RITE (en attente d'asset de Julien), 1-2 cartes "Keep" à couper jamais précisées, texture des 8 dés non-Blue.
...
- **`KEYWORDS`** : liste des mots-clés reconnus, calquée sur les mêmes strings que `tooltip.gd::get_tooltip_content()` attend déjà dans `tags` (Charge, Refuel, Scout, Boost, Blessed, Weak, Exposed, Lucky, Unlucky, Depleted, Energized, Strength, Muscle, Exhaust, Support, REST, + les 9 "X Dice"). Un mot-clé n'est coloré que s'il est listé dans le champ `tags` de la carte/relique — **sauf les types de dés** (voir plus bas).
