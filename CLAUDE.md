...
- **Cette session a couvert** : (1) refonte visuelle des tooltips (relic/dice/status) et du ScoutPanel, (2) refonte des vues Draw/Discard/Deck pile (header, séparateur, bouton Back, tooltips manquants), (3) plusieurs bugs de bordure/couleur sur les cartes (voir section dédiée `get_dynamic_description` plus bas et "Bugs récurrents"), (4) **nouveau système de description dynamique de carte** (`get_dynamic_description`) déployé sur ~31 cartes warrior (starters + pool draftable), (5) animation de vol carte-socketée→défausse pour les cartes jouées au dé rouge, (6) polish visuel de `dice_interface.tscn` (espacement des compteurs de dés), (7) nettoyage + restyle du `main_menu.tscn` (bug + UI morte supprimés, popup "Enable tutorial?" restylé, bouton Cancel ajouté), (8) **recherche de faisabilité pour un futur système de sauvegarde** (pas implémenté — voir section dédiée en bas de fichier, c'est le point de départ recommandé pour la prochaine session).
- **PROCHAINE SESSION — commencer ici si Julien parle de sauvegarde/continue** : section "Système de sauvegarde" tout en bas de ce fichier. Ne pas re-explorer `global.gd`/`character_stats.gd`/`run.gd` depuis zéro, le triage a déjà été fait.
