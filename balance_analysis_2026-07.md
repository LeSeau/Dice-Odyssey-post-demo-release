...
### Map/reward reassessment (2026-07-05, pre-tier-2-playtest)
Full re-audit of node weights and card-reward weights vs STS: **raw weights are already near-identical to STS1's documented tables** (M 46.6/Ev 25.4/C 12.7/El 8.5/Sh 6.8 vs STS 45/22/12/8/5) — improvements are structural, not numerical. Decisions from Julien:
- **Elite weight 1.0 → 1.2 APPLIED** (`map_generator.gd::ELITE_ROOM_WEIGHT`) — relic access; 1.5 rejected as overkill since events also grant relics.
- **Parked for later, explicitly acknowledged (don't re-propose, do remind when relevant)**: shop card-removal service (gold sink); rare/premium card attribute + populate (Julien: "definitely need", not now); campfire weight bump (gated on upgrade content shipping); event tier population (all 13 events are tier 0, `event_tier` plumbing exists but dead — deep-floor events same stakes as floor 2); tier-band widening via treasure-row move (rejected: Act 2 is coming, keep 3 thin tiers as-is).
- Reward-weight system (NORMAL 8.0 / SUPPORT 2.0, +0.3 pity per normal capped 5.0, reset on support) assessed healthy — matches the 48/12 pool composition, don't touch the base; the pity increment (0.3→0.5) is the lever if playtests show dice-manipulation droughts.
- **Dice economy fix APPLIED (2026-07-05, Julien picked option A only)**: `shop.gd::update_dice_price()` now applies a GLOBAL ×1.4 escalation per dice purchased (any type, `GLOBAL_DICE_PRICE_ESCALATION`, sums `Global.purchased_dice_counts`), replacing the old per-type-only ×1.35 which almost never fired (players diversify types → die #2 always at base price → affordable too early, per Julien's own playtest: 222g sitting around mid-run). Die #1 at base = intact happy milestone; die #2 ~1.4× = late-run stretch; die #3 ~2× = trophy. Options B (raise Green 150→180 floor), C (escalating reroll, currently flat 20g), D (retire the affordability blinker) proposed but NOT taken — revisit if playtests still show early second buys. ⚠️ Pre-existing gotcha, not fixed: `Global.purchased_dice_counts` is never reset on new run (autoload persists) — same latent issue the per-type escalation had; harmless while sessions restart per run, will matter once a real "new run" flow exists.
- **Key discovery from the economy audit**: the dice shop is NOT a map node — it's a top-bar button always accessible from the map, with an affordability blinker (`run.gd::_on_check_if_can_purchase_dice`) that starts flashing the moment the cheapest die is affordable. Deliberate core loop per Julien (most gold flows there); route-gating it was considered and rejected.
- **Map generation rebuilt STS-style (2026-07-05, approved by Julien after he correctly pushed back on "your maps are similar to STS")**. The similarity claim held for weights only, not algorithm — and an earlier stated STS monster share of 45% was wrong (STS monsters are the *remainder* after fixed proportions, ≈53%). Two structural mechanisms ported into `map_generator.gd`:
  1. **Quota-bag dealing replaces per-node i.i.d. rolls** (`_build_room_type_bag` + `_deal_type_from_bag`): the generator counts on-path random rooms, builds a fixed bag matching the weight proportions exactly (same tuning constants as before — weight/total = map share; monster = remainder), shuffles, deals. Every map now has near-identical composition (typical ~30-room map: 3 elites, 4 campfires, 2 shops, 8 events, rest monsters); randomness is arrangement only. No more shopless maps / elite droughts / event floods.
  2. **Sibling diversity** (`_get_assigned_sibling_types`, soft rule with two-pass relax + monster fallback): children of a shared parent avoid duplicate types when the bag allows, so forks now mostly present real choices instead of monster-vs-monster.
  - Hard rules preserved exactly: no elite/campfire before row 5, no campfire row 12, no same-type parent→child shop/elite/campfire. Treasure/campfire fixed floors now assigned BEFORE random dealing (were after — would have skewed quotas). Dealt top-down so early rows (which can't legally host elite/campfire) consume monsters/events/shops first. Dead code removed: `_set_room_randomly`, `_setup_random_room_weights`, `_get_random_room_type_by_weight`, the runtime weights dict. `_finalize_room` keeps the (run.gd-overridden) battle/event pre-assignments and the LIVE `is_secret_fight` 10% roll; its dead monster-tier threshold aligned to run.gd's `row > 7`.
  - ⚠️ NOT parse/runtime-tested (no Godot here). Verify by opening a few maps: node counts should be near-constant per map at the ratios above, and branch forks should rarely offer two identical room types.
